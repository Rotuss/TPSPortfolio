...
void ATPSCharacter::Tick(float DeltaTime)
{
    ...
    //ENetRole Test = GetLocalRole();
    //UE_LOG(LogTemp, Warning, TEXT("Test : %s, %d"), *GetName(), Test);
...
void ATPSCharacter::Walk(const FInputActionValue& Value)
{
    ...
    {
        if (true == UKismetSystemLibrary::IsServer(this))
        {
            //GetNetMode();
            UE_LOG(LogTemp, Warning, TEXT("GetNetmode() : %d"), GetNetMode());
            ServerToClient_Walk();
        }
        //ENetRole Test = GetLocalRole();
        //UE_LOG(LogTemp, Warning, TEXT("Test : %d"), Test);
        if (false == UKismetSystemLibrary::IsServer(this))
        {
            UE_LOG(LogTemp, Warning, TEXT("GetNetmode() : %d"), GetNetMode());
            Walk_Speed();
        }
...
void ATPSCharacter::Walk_Speed()
{
    GetCharacterMovement()->MaxWalkSpeed = 300.0f;
}

void ATPSCharacter::Run_Speed()
{
...
void ATPSCharacter::ServerToClient_Walk_Implementation()
{
    Walk_Speed();
}

void ATPSCharacter::ServerToClient_Run_Implementation()
{
    Run_Speed();
}